...
La variable `mode` hace referencia a
...
| `valence`          | numérica | Que tipo de emociónes transmite la canción, más triste cerca de 0, más feliz cerca de 1                                                                |
| `acousticness`     | numérica | Que tan acustico es el sonido de la canción (precencia de instrumentos no electricos ni digitales), mas acústico cerca de 1, menos acústico cerca de 0 |
...
| `mode`             | numérica | Canción considerada como prioritaria por el artista o sello discográfico, se suele publicitar y recomendar más (Si=1, No=0)                            |
| `popularity`       | numérica | Que tan popular es la canción                                                                                                                          |
| `speechiness`      | numérica | Proporcion de tiempo que ocupan palabras habladas o cantadas en la canción                                                                             |
...
grafico1=ggplot(data = datos)+
  ...
  labs(title="Figura 2:Número de canciones según su popularidad",
       x="Popularidad", y="Número de Canciones")+
  geom_vline(xintercept = 74,linetype="dashed",color="black")+
  annotate("text", x = 91, y = 25000, label = "Canciones a estudiar")
grafico1
```
